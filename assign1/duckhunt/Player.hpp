...
private:
    std::vector<int> getBirdMovementSeq(const Bird& bird, uint start=0,uint end=100);
    std::vector<std::vector<hmm::HMM*>> speciesModels;
    ESpecies guessBirdSpecs(const Bird& bird);
    // int shooterTimer = 0;
    int nShoot = 0;
    int nHit = 0;
